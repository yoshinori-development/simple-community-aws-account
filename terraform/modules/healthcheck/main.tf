...
resource "aws_cloudwatch_metric_alarm" "app-console-health-check" {
  alarm_name          = "${local.infra_fullname}-app_console_health_check"
  comparison_operator = "LessThanThreshold"
  evaluation_periods  = "1"
  metric_name         = "HealthCheckStatus"
  namespace           = "AWS/Route53"
  ...
  statistic           = "Minimum"
  threshold           = "1"
  alarm_description   = "This metric monitor app-console url healthcheck"
  alarm_actions       = [module.health_check_notify_slack.this_slack_topic_arn]
  ok_actions          = [module.health_check_notify_slack.this_slack_topic_arn]
  dimensions = {
    HealthCheckId = aws_route53_health_check.app-console.id
  }
}

resource "aws_route53_health_check" "app-console" {
  fqdn              = var.console_fqdn
  port              = 443
  type              = "HTTPS"
  resource_path     = "/health"
  failure_threshold = "5"
